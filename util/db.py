...
class mydb:
    ...
    def create_db(self):
        try:        
            ...
            cur.execute("CREATE DATABASE IF NOT EXISTS %s DEFAULT CHARSET utf8"%(cf['db_name']))
            self.conn.commit()
        except Exception as err:
            logging.error(err)
    ...
    def drop_db(self):
        try :
            cur = self.conn.cursor()
            cur.execute("drop database if exists %s;"%(cf['db_name']))
            self.conn.commit()
        except Exception as err:
            logging.error(err)
    ...
    def execute_sql(self,sql_str):
        try :
            cur = self.conn.cursor()
            cur.execute("use %s"%(cf['db_name']))
            cur.execute(sql_str)
            self.conn.commit()
        except Exception as err:
            logging.error(err)

    def insert_sql(self,sql_str,commit_num = 1):
        try :
            self.count += 1
            cur = self.conn.cursor()
            cur.execute("use %s"%(cf['db_name']))
